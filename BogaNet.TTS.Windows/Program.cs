    ...
    internal class Program
    {
       #region Variables
        ...
        private static void voices()
        {
            ...
            using (SpeechSynthesizer speechSynthesizer = new SpeechSynthesizer())
            {
                ...
                foreach (InstalledVoice installedVoice in installedVoices)
                {
                    ...
                    {
                        voice = "@VOICE:" + installedVoice.VoiceInfo.Name + ":" + installedVoice.VoiceInfo.Description + ":" + installedVoice.VoiceInfo.Gender + ":" + installedVoice.VoiceInfo.Age + ":" + installedVoice.VoiceInfo.Culture;
                        writeOut(voice);
                    }
                    else
                    {
                        writeOut("WARNING: Voice is disabled: " + installedVoice.VoiceInfo.Name + ":" + installedVoice.VoiceInfo.Description + ":" + installedVoice.VoiceInfo.Gender + ":" + installedVoice.VoiceInfo.Age + ":" + installedVoice.VoiceInfo.Culture);
                    }
                }
            }
        ...
        private static void speak()
        {
            ...
            using (SpeechSynthesizer speechSynthesizer = new SpeechSynthesizer())
            {
                speechSynthesizer.SpeakProgress += new EventHandler<SpeakProgressEventArgs>(synthSpeakProgress);
                speechSynthesizer.PhonemeReached += new EventHandler<PhonemeReachedEventArgs>(synthPhonemeReached);
                speechSynthesizer.VisemeReached += new EventHandler<VisemeReachedEventArgs>(synthVisemeReached);
        ...
        private static void speak(string speechText, SpeechSynthesizer speechSynthesizer)
        {
            try
            {
                ...
                {
                    writeOut("Speech SSML text: '" + speechText/*.Replace('\'', '"')*/ + "'"); //TODO remove?

                    //speechSynthesizer.SpeakSsml(speechText.Replace('\'', '"'));
                    speechSynthesizer.SpeakSsml(speechText);
                }
                else
                {
                    writeOut("Speech text: '" + speechText + "'"); //TODO remove?
        ...
        private static void writeFile(string filename, string text)
        {
            if (!string.IsNullOrEmpty(filename))
            {
                ...
                {
                    writeErr("Could not write file: " + ex);
                }
            }
        }
        */
        ...
        private static void synthPhonemeReached(object sender, PhonemeReachedEventArgs e)
        {
            writeOut("@PHONEME:" + e.Phoneme);

            /*
            writeOut(string.Format(
        ...
        private static void synthVisemeReached(object sender, VisemeReachedEventArgs e)
        {
            writeOut("@VISEME:" + e.Viseme);

            /*
            writeOut(string.Format(
                "  Viseme reached summary:" + Environment.NewLine +
                "    Viseme:                 {0}" + Environment.NewLine +
                "    Audio Position:         {1} mSec" + Environment.NewLine +
                "    Duration:                  {2} mSec" + Environment.NewLine +
                "    Emphasis:                {3}" + Environment.NewLine +
                "    Next Viseme:         {4}" + Environment.NewLine + Environment.NewLine,
                e.Viseme,
                e.AudioPosition,
                e.Duration,
                e.Emphasis,
        ...
        private static void synthSpeakProgress(object sender, SpeakProgressEventArgs e)
        {
            writeOut("@WORD:" + e.Text);
        }
